...
const PropertyQuiz: React.FC<PropertyQuizProps> = ({
  mode = 'inline',
  onClose,
  className
}) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const filteredQuestions = getFilteredQuestions(userType);
      const formattedAnswers = Object.entries(answers).map(([questionIdStr, answer]) => {
  ...
  if (isSubmitted) {
    return <div className={mode === 'fullscreen' ? "fixed inset-0 bg-gradient-to-r from-slate-900 to-estate-dark-blue z-50 overflow-y-auto" : ""}>
        <div className="min-h-screen flex flex-col items-center justify-center p-4">
          <Card className={cn("w-full max-w-lg text-center bg-white", mode === 'fullscreen' ? "bg-white/10 border-white/20 text-white" : "", className)}>
            <CardHeader>
            ...
            <CardContent>
              ...
              <Button onClick={() => navigate('/')} className="bg-estate-blue hover:bg-estate-dark-blue text-white">
                Return Home Now
              </Button>
            </CardContent>
          </Card>
  ...
  return <Card className={cn("w-full bg-white border shadow-md", className)}>
      <CardHeader>
        <div className="flex items-center gap-3 mb-2">
          <MessageSquare size={24} className="text-estate-blue" />
          <CardTitle>
            {isContactInfoScreen ? "Almost Done!" : userType === null ? "Commercial Property Questionnaire" : userType === 'buyer' ? "Buyer Questionnaire" : "Seller Questionnaire"}
          </CardTitle>
        </div>
        <CardDescription className="text-base">
          {isContactInfoScreen ? "Please provide your contact information so our agent can get in touch with you." : getCurrentQuestion()?.description || "Please help us understand your needs better."}
        </CardDescription>
      </CardHeader>
      ...
      <CardContent>
        <div className="mb-6">
          <h3 className="text-lg font-semibold mb-4">
            {isContactInfoScreen ? "Your Contact Information" : getCurrentQuestion()?.question}
          </h3>
          <QuizContent currentQuestion={getCurrentQuestion()} answers={answers} setAnswers={setAnswers} name={name} setName={setName} email={email} setEmail={setEmail} phone={phone} setPhone={setPhone} handleNext={handleNext} handleCheckboxChange={handleCheckboxChange} handleRadioChange={handleRadioChange} handleSelectChange={handleSelectChange} isCheckboxSelected={isCheckboxSelected} mode={mode} isAuthenticated={!!user} showAuthDialog={() => setShowAuthDialog(true)} />
        </div>
        
        <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
          <div className="bg-estate-blue h-full rounded-full transition-all duration-300 ease-in-out" style={{
          width: `${progress}%`
        }} />
        </div>
      </CardContent>
      ...
      <CardFooter className="flex justify-between border-t pt-4">
        {!isFirstQuestion && <Button variant="outline" onClick={handlePrevious} className="border-gray-200 bg-gray-100 hover:bg-gray-200">
            <ArrowLeft className="mr-2 h-4 w-4" />
            Back
          </Button>}
        
        {!isLastQuestion ? <Button onClick={handleNext} disabled={!canProceed()} className="ml-auto bg-estate-blue hover:bg-estate-dark-blue text-white">
            Next
          </Button> : <Button onClick={handleSubmit} disabled={isSubmitting || !canProceed()} className="ml-auto bg-estate-blue hover:bg-estate-dark-blue text-white">
            {isSubmitting ? <>
